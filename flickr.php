...
function getPhotoEXIFById($fObj, $photoId)
{
	...
	if ($rawEXIF["camera"]) {
		array_push($photoEXIF, array(
			"order" => 0,
			"item" => "照相机型号",
			"meta" => "model",
			"content" => $rawEXIF["camera"],
			"display" => $rawEXIF["camera"]
		));
					...
					case "FNumber":
						...
						if (noDuplicatedItems($photoEXIF, $itemName)) {
							array_push($photoEXIF, array(
								...
								"item" => $itemName,
								"meta" => $itemMeta,
								"content" => $rawEXIFitem["clean"]["_content"],
								"display" => $rawEXIFitem["clean"]["_content"]
							));
						}
						break;
					case "FocalLength":
						...
						if (noDuplicatedItems($photoEXIF, $itemName)) {
							array_push($photoEXIF, array(
								...
								"meta" => $itemMeta,
								"content" => $EXIFcontent,
								"display" => str_replace(" mm", "毫米", $EXIFcontent)
							));
						}
						break;
					case "Flash":
...
function getPhotoById($fObj, $photoId, $photoSize = NULL, $inAlbums = false, $primarySize = NULL, $withEXIF = false, $interestingnessEvaluation = false)
{
	...
	$photoInfo = array(
		...
		"dates" => array(
			"taken" => timetotxt($photo["dates"]["taken"]),
			"posted" => timetotxt($photo["dates"]["posted"], true)
		),
		"views" => $photo["views"]
	);
...
function getAlbumById($fObj, $albumId, $primarySize = NULL, $withContents = false, $photoSize = NULL)
{
	...
	$albumInfo = array(
		...
		"count" => array(
			"photos" => $album["count_photos"],
			"videos" => $album["count_videos"]
		),
		"views" => $album["count_views"],
		"dates" => array(
			"created" => timetotxt($album["date_create"], true),
			"updated" => timetotxt($album["date_update"], true)
		),
		"primary" => $primary
	);
